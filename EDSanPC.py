...
def run_error_injection_oracle_pc(benchmark, error_rate=0.1, seed :int=0):
...
if __name__ == "__main__":
    ...
    result = {"error_detected": [], "error_count": [], "ci_count": [], "error_injection_position": [], "last_time": [],
              "args": f"{args}"} 
    for seed in range(10):
        start_time = datetime.now()
        dag_path = f"/home/zjiae/Data/benchmarks/{benchmark}_graph.txt"
        dag=read_dag(dag_path)
        error_detected, error_count, ci_count, error_injection_position = run_error_injection_oracle_pc(benchmark, 0.05, seed)
    ...
    json.dump(result, open(f"result/{benchmark}_{args.use_marginal}_{args.use_graphoid}_{args.use_slicing}_EDSanPC.json", "w")) 
